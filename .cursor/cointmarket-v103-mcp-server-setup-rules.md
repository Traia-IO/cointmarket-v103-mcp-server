# Cointmarket v103 MCP Server Implementation Guide

You are working on implementing the Cointmarket v103 MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: Cointmarket v103
- **Documentation**: []()
- **Website**: [https://pro-api.coinmarketcap.com/](https://pro-api.coinmarketcap.com/)
- **Authentication**: Required - API key via `COINTMARKET_V103_API_KEY` environment variable
...
When a client provides their own Cointmarket v103 API key:
...
Flow:
  1. Client connects with their Cointmarket v103 API key
  2. MCP server uses client's API key to call Cointmarket v103 API
  3. No payment required
  4. Client pays Cointmarket v103 directly (not the MCP server)
```

**Use Case**: Clients who already have a Cointmarket v103 subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL Cointmarket v103 API key to call the API
  5. Client pays the MCP server (not Cointmarket v103)
```

**Use Case**: Pay-per-use clients without their own Cointmarket v103 subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("COINTMARKET_V103_API_KEY")
...
**Required**:
- `COINTMARKET_V103_API_KEY`: Server's internal Cointmarket v103 API key (used when clients pay via 402)
...
COINTMARKET_V103_API_KEY=your_cointmarket-v103_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_cointmarket_v103",
      "get_cointmarket_v103_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["cointmarket v103", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the Cointmarket v103 API with retry support. Example pattern:
...
def call_cointmarket_v103_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call Cointmarket v103 API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"Cointmarket v103 API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_cointmarket_v103(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search Cointmarket v103 for [specific data type].
    ...
    try:
        # The call_cointmarket_v103_api function already has retry logic
        results = call_cointmarket_v103_api(
...
```python
@mcp.tool()
async def get_cointmarket_v103_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_cointmarket_v103_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in Cointmarket v103.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_cointmarket_v103(query="example", limit=5)
   ```
...
### 9. Common Cointmarket v103 Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on Cointmarket v103 capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the Cointmarket v103 documentation that you should implement:
...
- Check the Cointmarket v103 API documentation: 
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make Cointmarket v103's capabilities easily accessible to AI agents through standardized MCP tools. 