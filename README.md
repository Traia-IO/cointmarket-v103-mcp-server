...
This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Cointmarket v103 API. It enables AI agents and LLMs to interact with Cointmarket v103 through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Cointmarket v103 endpoints
...
- **Cointmarket v103 Website**: [https://pro-api.coinmarketcap.com/](https://pro-api.coinmarketcap.com/)
- **API Documentation**: []()
...
*Note: Replace `example_tool` with actual Cointmarket v103 API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/cointmarket-v103-mcp-server.git
   cd cointmarket-v103-mcp-server
   ```

2. Set your API key:
   ```bash
   export COINTMARKET_V103_API_KEY="your-api-key-here"
   ```
...
# Server's internal API key (for payment mode)
   COINTMARKET_V103_API_KEY=your-api-key-here
...
2. Run the server:
   ```bash
COINTMARKET_V103_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own Cointmarket v103 API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_COINTMARKET_V103_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their Cointmarket v103 API key
2. Server uses client's API key to call Cointmarket v103 API
3. No payment required
4. Client pays Cointmarket v103 directly
...
6. Server uses its INTERNAL Cointmarket v103 API key to call the API
...
```bash
# Required
COINTMARKET_V103_API_KEY=your_internal_cointmarket-v103_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Cointmarket v103 API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Cointmarket v103 endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/cointmarket-v103-mcp-server",
  "mcp_server": {
    "name": "cointmarket-v103-mcp",
    "description": "Another mcp for test",
...
- `COINTMARKET_V103_API_KEY`: Your Cointmarket v103 API key (required)
## Troubleshooting
