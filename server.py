#!/usr/bin/env python3
"""
Cointmarket v103 MCP Server - FastMCP with D402 Transport Wrapper
...
- COINTMARKET_V103_API_KEY: Server's internal API key (for paid requests)
...
logger = logging.getLogger('cointmarket-v103_mcp')
...
API_KEY = os.getenv("COINTMARKET_V103_API_KEY")
if not API_KEY:
    logger.warning(f"⚠️  COINTMARKET_V103_API_KEY not set - payment required for all requests")

logger.info("="*80)
logger.info(f"Cointmarket v103 MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("Cointmarket v103 MCP Server", host="0.0.0.0")
...
# - COINTMARKET_V103_API_KEY: Server's internal API key for payment mode
#
...
async def coinmarketcap_id_map_i3nb(
    context: Context,
    listing_status: str = "active",
    slug: Optional[str] = None,
    start: int = 1,
    ...
    Example Usage:
        await coinmarketcap_id_map_i3nb()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_i3nb: {e}")
...
async def coinmarketcap_id_map_ko6m(
    context: Context,
    start: int = 1,
    limit: Optional[int] = None,
    sort: str = "id",
    ...
    Example Usage:
        await coinmarketcap_id_map_ko6m()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_ko6m: {e}")
...
async def quotes_historical_nai7(
    context: Context,
    time_start: Optional[str] = None,
    time_end: Optional[str] = None,
    count: float = 10,
    ...
    try:
        ...
        logger.error(f"Error in quotes_historical_nai7: {e}")
...
async def quotes_latest_8ygj(
    ...
    try:
        ...
        logger.error(f"Error in quotes_latest_8ygj: {e}")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="cointmarket-v103-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "cointmarket-v103-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Cointmarket v103 MCP Server")
    logger.info("="*80)
